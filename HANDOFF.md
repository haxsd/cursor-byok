...
## 十九、窗口默认尺寸与「弹窗错位」的真相（2026-09-23）

### 用户报的两件事

1. 点「使用教程」后界面像坏了：弹窗跑到右下角、按钮看不见。
2. 主窗口默认太小，顶部一行显示不全。

### 第 2 件是真的，已修

`desktop.rs` 原来写死 `inner_size(820, 558)`（逻辑像素）。这台机器是 1536×960 逻辑
像素（250% 缩放，3840×2400 物理），窗口只占半屏，导航条右侧的状态区与 `Ctrl K`
被裁掉，概览四磁贴挤成两列。

现在按主显示器 `work_area()`（已扣任务栏）减每边 96px，上限 1320×880、下限 880×600，
`min_inner_size` 从 820×558 放宽到 880×600。实测 **820×558 → 1320×816**。

`app.windows` 在 `tauri.conf.json` 里是空数组——窗口是 Rust 建的，改尺寸别去翻配置。

### 第 1 件是我自己造成的，不是产品缺陷

**根因：CDP 的 `Emulation.setDeviceMetricsOverride` 会留在目标上。** 我的截图工具默认把
视口覆盖成 1500×950 再截图，用完不清理。对一次性的预览浏览器无所谓，但同一套工具现在
也用来连**已安装应用**的 WebView2 调试端口——覆盖留在那里，整个界面就按 1500×950 排版：
内容看着整体下移，居中的弹窗落到可见区域（820×558）的右下角，按钮在窗口外。

实测证据（真实应用、去掉覆盖后）：遮罩铺满 820×558，弹窗居中在 (100,188)，尺寸
620×188，footer 两个按钮都在；换到 1320×816 后弹窗在 (350,317)，footer 底边 504 < 816。

**已修工具**：`shot.mjs`、`shot-routes.mjs`、`zoom-shot.mjs`、`render-theme.mjs`、
`measure-layout.mjs` 在断开前都执行 `Emulation.clearDeviceMetricsOverride`。
**以后加新的 CDP 工具，记得照做。**

### 另一个差点被当成 bug 的现象

概览页一度显示范围选中「今天」，而代码默认值是「month」。重载后读到
`30 天 / 近一个月`——默认值是对的，那次是界面上真的被点过。
**结论：状态类问题先重载再判定，别凭一次读数下结论。**

---

## 二十、使用教程改成本产品自己的页面（2026-09-23）

### 之前是什么

导航条最后那个带红点的「使用教程」是**外链按钮**，点开确认框后打开
`https://docs.leokun.cn`——**上游产品的文档站**。那里的界面、步骤和概念都不是这个
应用的（它是分叉来源那一侧的文档），用户照着做会对不上。用户要求重做，已改。

### 现在是什么

`features/tutorial/TutorialPage.tsx`，路由 `/tutorial`，是一个正常的页内页面：

| 段落 | 内容 |
|---|---|
| 顶部 | 一句话说明这是什么 + **上手进度 N/4**（读真实状态） |
| 上手清单 | 四步：加模型 / 让 Cursor 用上它 / 让 Devin 用上它 / 跑一次对话。每步给完成状态与现状，未完成的给「去哪一页」按钮 |
| 请求怎么走 | 客户端 → 本机网关 → 你的模型（复用 `ConnectionPath`），并说明只有模型请求被本地处理 |
| 常见问题 | 八条 `<details>`：模型测试失败、Cursor 看不到模型、端口未监听、缓存命中率为 0、计价口径、**退出软件后客户端还能用吗**（关键：接管期间软件不在，请求会失败）、是否影响同类软件、数据在哪怎么清 |
| 术语表 | BYOK / 接管 / 本地 CA / 宿主补丁 / 缓存命中率 / 详细模式 |
| 还能去哪 | 四条排查入口，指向调用页、设置页、模型库、Devin 页 |

状态判定：`models.length`、`cursorHarness.ca`+`settings_applied`、`devinStatus`+
`api.devinHostStatus()`、`calls.length`。**教程里写的每一句都要和界面实际行为一致**，
所以这些判断直接读同一份状态，不写死文案。

「未读」小圆点改成「还没打开过这一页」（localStorage 仍是 `haxsd-byok:tutorial-read`）。
模型库空状态加了「先看使用教程」入口。

### 顺带修掉的英文布局问题

英文导航标签比中文长：窗口 1320px 时导航条放不下，最后一项被压成一个字母（截图里
只剩 "U"）。两处修：CSS 让标签省略、图标与状态保留（`.navItem` 的 `min-width: 0` +
标签 `text-overflow: ellipsis`）；英文标签改短（Overview / Models / Settings / Guide）。
这些标签与页面标题共用同一个词条，所以页面标题也跟着变——读起来仍然通顺。

### 工具

`check-overflow.mjs` 与 `audit-a11y.mjs` 的路由表加上了 `/tutorial`（新页面要进检查范围）。
教程页在这两项检查里都是干净的（无横向溢出、1 个 main、1 个 h1、控件都有名字）。

安装验证：已重新构建并安装 1.0.5，真实应用里教程显示 **3/4**（模型 ✓、Devin 已接通 ✓、
8 条调用记录 ✓，Cursor 那步因为这台机器没初始化 CA 而停在第二步——状态如实反映）。
截图：`app-1.0.5-tutorial.png`、`tut-tutorial.png`、`tut-tutorial-faq.png`、
`tut-tutorial-en2.png`。
